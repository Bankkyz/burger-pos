...
function StockBadge({ ingredient }: { ingredient: Ingredient }) {
      ...
      <table className="w-full min-w-[720px] border-collapse text-sm">
        <thead>
          <tr className="border-b border-[var(--color-border)] text-left text-[var(--color-text-muted)]">
            ...
            <th className="whitespace-nowrap px-3 py-2.5 font-medium">Cost / g</th>
            <th className="whitespace-nowrap px-3 py-2.5 font-medium">Supplier</th>
            <th className="whitespace-nowrap px-3 py-2.5 font-medium">Expires</th>
            ...
            <tr key={ingredient.id} className="border-b border-[var(--color-border)] last:border-0">
              ...
              <td className="whitespace-nowrap px-3 py-3">
                <div className="flex justify-end gap-1">
                  <Button variant="ghost" size="icon" aria-label={`Edit ${ingredient.name}`} onClick={() => onEdit(ingredient)}>
                    <Pencil className="h-4 w-4" />
                  </Button>
                  <Button variant="ghost" size="icon" aria-label={`Delete ${ingredient.name}`} onClick={() => onDelete(ingredient)}>
