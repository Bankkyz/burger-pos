  ...
  return (
    <div className="flex h-full flex-col gap-4">
      <div className="flex items-center gap-2">
        ...
        <h2 className="font-semibold text-[var(--color-text)]">{t.sales.currentOrder}</h2>
      </div>

      <div className="flex-1 overflow-y-auto scrollbar-thin">
        {lines.length === 0 ? (
          <p className="py-8 text-center text-sm text-[var(--color-text-muted)]">{t.sales.tapToAdd}</p>
        ) : (
              ...
              <li key={line.recipeId} className="flex items-center gap-2">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-[var(--color-text-muted)]">
                    {formatCurrency(line.unitPrice)} {t.sales.each}
                  </p>
                </div>
                <div className="flex items-center gap-1.5">
                  <Button
                    ...
                    size="icon"
                    className="h-8 w-8"
                    aria-label={`${t.common.decrease} ${line.recipeName}`}
                    onClick={() => onDecrement(line.recipeId)}
                  >
                  ...
                  <Button
                    ...
                    size="icon"
                    className="h-8 w-8"
                    aria-label={`${t.common.increase} ${line.recipeName}`}
                    onClick={() => onIncrement(line.recipeId)}
                  >
                ...
                <Button
                  ...
                  size="icon"
                  className="h-8 w-8"
                  aria-label={`${t.common.remove} ${line.recipeName}`}
                  onClick={() => onRemove(line.recipeId)}
                >
      ...
      <div className="flex flex-col gap-3 border-t border-[var(--color-border)] pt-4">
        <div className="grid grid-cols-3 gap-2">
          <Input label={t.sales.discount} type="number" min={0} step="0.01" value={discount} onChange={(e) => onDiscountChange(Number(e.target.value) || 0)} />
          <Input label={t.sales.service} type="number" min={0} step="0.01" value={serviceCharge} onChange={(e) => onServiceChargeChange(Number(e.target.value) || 0)} />
          <Input label={t.sales.delivery} type="number" min={0} step="0.01" value={deliveryFee} onChange={(e) => onDeliveryFeeChange(Number(e.target.value) || 0)} />
        </div>

        <div className="grid grid-cols-2 gap-2">
          <Select label={t.sales.payment} value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value as PaymentMethod)}>
          ...
          <Select label={t.sales.channel} value={channel} onChange={(e) => setChannel(e.target.value as SaleChannel)}>
            {SALE_CHANNELS.map((c) => (
